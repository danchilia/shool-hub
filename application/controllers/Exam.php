...
class Exam extends Admin_Controller
{
    ...
    protected function exam_validation()
    {
        ...
        $this->form_validation->set_rules('grading_system', 'Grading System', 'trim|required');
        $gradingSystem = $this->input->post('grading_system');
    ...
    public function mark_entry()
    {
        ...
        if (!empty($examID)) {
            $examRow = $this->db->get_where('exam', array('id' => $examID))->row();
            if ($examRow && $examRow->grading_system == 'cbc') {
                redirect(base_url('cbc/assessment'));
            }
        }
