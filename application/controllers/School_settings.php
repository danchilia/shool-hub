...
class School_settings extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $logoTypes = array(
                    'branch_printing_logo' => 'printing_logo',
                    'branch_report_logo' => 'report_card_logo',
                    'branch_logo' => 'logo',
                );
                $uploadPath = FCPATH . 'uploads/branch_logos/';
                if (!is_dir($uploadPath)) {
                    mkdir($uploadPath, 0755, true);
                }
                foreach ($logoTypes as $inputName => $dbColumn) {
                    if (isset($_FILES[$inputName]) && !empty($_FILES[$inputName]['name'])) {
                        $config = array(
                            'upload_path' => $uploadPath,
                            'allowed_types' => 'png|jpg|jpeg',
                            'max_size' => 2048,
                            'file_name' => 'branch_' . $branchID . '_' . $dbColumn,
                            'overwrite' => true,
                        );
                        ...
                        if ($this->upload->do_upload($inputName)) {
                            ...
                            $this->db->update('branch', array($dbColumn => $fileName));
                        }
                    }
                }
    ...
    public function mpesa_save()
    {
        ...
        $q = $this->db->get('payment_config');
        if ($q->num_rows() == 0) {
            $arrayMpesa['branch_id'] = $branchID;
            $this->db->insert('payment_config', $arrayMpesa);
        } else {
            $this->db->where('id', $q->row()->id);
            $this->db->update('payment_config', $arrayMpesa);
        }
        $message = translate('the_configuration_has_been_updated');
        $array = array('status' => 'success', 'message' => $message);
        echo json_encode($array);
    ...
    public function payment_active()
    {
        ...
        $mpesa_status = isset($_POST['mpesa_status']) ? 1 : 0;
        $arrayData = array(
    ...
    public function africastalking_save()
    {
        if (!get_permission('sms_settings', 'is_add')) {
            ajax_access_denied();
        }
        $branchID = $this->school_model->getBranchID();
        $this->form_validation->set_rules('at_username', 'Username', 'trim|required');
        $this->form_validation->set_rules('at_api_key', 'API Key', 'trim|required');
        if ($this->form_validation->run() !== false) {
            $data = array(
                'sms_api_id' => 6,
                'field_one' => $this->input->post('at_username'),
                'field_two' => $this->input->post('at_api_key'),
                'field_three' => $this->input->post('at_sender_id'),
                'field_four' => '',
                'branch_id' => $branchID,
                'is_active' => 0,
            );
            $this->db->where(array('sms_api_id' => 6, 'branch_id' => $branchID));
