								...
								<div class="table-responsive">
									<table class="table table-bordered table-hover table-condensed tbr-top mb-none">
										<thead>
											<tr>
												<th>#</th>
												<?php if (is_superadmin_loggedin()): ?><th><?=translate('branch')?></th><?php endif; ?>
										...
										<tbody>
											<?php
											$count = 1;
											$levelLabels = array('pp'=>'Pre-Primary','lower_primary'=>'Lower Primary','upper_primary'=>'Upper Primary','junior_secondary'=>'Junior Secondary','senior_secondary'=>'Senior Secondary');
											if (count($classlist)){
												foreach($classlist as $row):
													$ct = isset($row['curriculum_type']) ? $row['curriculum_type'] : '844';
													$lvl = isset($row['level']) && isset($levelLabels[$row['level']]) ? $levelLabels[$row['level']] : '-';
											?>
											<tr>
												<td><?php echo $count++;?></td>
												<?php if (is_superadmin_loggedin()): ?><td><?php echo $row['branch_name'];?></td><?php endif; ?>
												<td><?php echo $row['name'];?></td>
												<td><?php echo $ct == 'cbc' ? '<span class="label label-info">CBC</span>' : '<span class="label label-default">8-4-4</span>'; ?></td>
												<td><?php echo $lvl; ?></td>
												<td>
													<?php
